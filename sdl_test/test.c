...
int main(int argc, char *argv[]) {
	...
	screenx = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STREAMING, 320, 200);
	if (screenx) {
		printf("Created!\n");
	}
	...
	Uint32 format;
	int access;
	SDL_QueryTexture(screenx, &format, &access, &w, &h);

	printf("Format %u Access %u W: %d H: %d, %s\n", format, access, w, h, SDL_GetPixelFormatName(format));
